...
exports.init = function(pluginInterface) {
    ...
    pi.setOnGetSettingsSchemaCallback(function() {
    ...
    pi.setOnSettingsUpdatedCallback(function(newSettings) {
        return new Promise((ac, rj)=>{
...
/**
 * Settings procedure
 * @param {object} pluginInterface Plugin interface
 */
function settingsProc(pluginInterface) {
    const pi = pluginInterface;
    const localStorage = pi.localStorage;
    ...
    const schemaJson = readJSON('settings_schema.json');
    const schemaDefaultJson = readJSON('settings_schema_default.json');
    const schemaWlanJson = readJSON('settings_schema_wlan.json');

    let clSettings = localStorage.content();
    let curSettings;
    try {
        curSettings = readJSON('settings.json');
    } catch (e) {
        curSettings = {};
    };

    if (typeof clSettings == 'object') {
        curSettings.api_filter = {};
        let setProp = {};// schema_json.properties;
        for (const clname of Object.keys(clSettings)) {
            setProp[clname] = {
                title: clname+' client allowed path in regexp',
                type: 'string',
            };
            if (clSettings[clname].filter != null) {
                setProp[clname].default = clSettings[clname].filter;
            }

            curSettings.api_filter[clname] =
                clSettings[clname].filter;
        }
        schemaJson.properties.api_filter.properties = setProp;
        // fs.writeFileSync(pi.getpath()+'settings.json',JSON.stringify(cur_settings,null,'\t'));
    }
...
"$ sudo apt-get purge dhcpcd5"`,
            ...
            if (curInterf != undefined &&
                interfaces.indexOf[curInterf]<0) {
            ...
            if (!bWlanExist) {
                return schemaJson;
                // log(JSON.stringify(schemaJson,null,'\t'));
            } else { // WiFi scan
